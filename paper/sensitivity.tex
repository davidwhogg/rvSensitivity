...
\begin{align}
  ...
  \hquad,
\end{align}
where $\vu_\kappa$ is the unit vector that ``points in the direction of''
or projects out the velocity amplitude $\kappa$,
and $\tC_\theta$ is the inverse\footnote{CITE
  Albrecht here, and also note that you shouldn't invert in general,
  but in low-d problems like this it is fine.} of the precision matrix $\tC_\theta\inv$.
